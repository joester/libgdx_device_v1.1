...
public class SpawnList extends GuiObject{
    ...
    public static ArrayList<SpawnLocation> selectedSpawns;
	...
	new SpawnList_ToggleTimeBars().register();
    }
    ...
    /////Selection
    static SpawnLocation getSpawnAtIndex(int index){
	...
	SpawnLocation spawn = getSpawnAtIndex(index);
	if(spawn != null){
	    selectSpawn(spawn);
	    if(firstSelection == -1)
    ...
    public static void selectSpawn(SpawnLocation spawn){
    ...
    /////Draw
    String getTimeTextFromSpawnLocation(SpawnLocation spawn){
	String s = ""+spawn.time+'s';
	for(int i = 0; i < 7-s.length(); i++){
	    s += ' ';
	}
	return s;
    }
    String getNameTextFromSpawnLocation(SpawnLocation spawn){
    ...
    void drawSingleUnit(int realindex,int relativeindex,SpawnLocation spawn){
	Vector2 pos = new Vector2(OFFSET.x+BARSIZE.x/2,OFFSET.y+BARSIZE.y*(.5f+relativeindex));
	
	//Draw a bar between spawn locations that are at different times
	if(realindex == 0 || (int)spawn.time != (int)SpawnMap.currentMap.spawns.get(realindex-1).time){
	    GraphicsDraw.setColor(Color.BLACK);
	    if(showTimeBars){
		GraphicsDraw.fillRectangle(new Vector2(pos.x+10,pos.y-BARSIZE.y/2+1),new Vector2(BARSIZE.x+20,3));
		GraphicsDraw.centerText(""+(int)spawn.time+" s",new Vector2(pos.x+BARSIZE.x/2+35,pos.y-BARSIZE.y/2));
	    }else{
		GraphicsDraw.fillRectangle(new Vector2(pos.x,pos.y-BARSIZE.y/2+1),new Vector2(BARSIZE.x,3));
	    }
	}
	...
	/////Draw text
	GraphicsDraw.centerText(getTimeTextFromSpawnLocation(spawn),new Vector2(pos.x-BARSIZE.x/5,pos.y));
...
class SpawnList_Scrollbar extends GuiObject{
	...
	return (int)((screenposition-position.y)/size.y*(SpawnMap.currentMap.spawns.size()-SpawnList.MAXBARS));
    }
...
class SpawnList_ToggleTimeBars extends GuiButton{
    public SpawnList_ToggleTimeBars(){
	setSize(new Vector2(SpawnList.ARROWSQUARESIZE/2,SpawnList.ARROWSQUARESIZE/2));
	setPosition(new Vector2(SpawnList.OFFSET.x+SpawnList.SIZE.x+30,SpawnList.OFFSET.y-SpawnList.ARROWSQUARESIZE));
    }
    ...
    public Color getBaseColor(){
	return SpawnList.instance.showTimeBars ? COLOR_ON : COLOR_OFF;
    }
    
    public void button_OnMouseClick(){
	SpawnList.instance.showTimeBars = !SpawnList.instance.showTimeBars;
    }
	...
	GraphicsDraw.centerText("show times",new Vector2(getCenter().x,getCenter().y-size.y/2-5));
    }
}
