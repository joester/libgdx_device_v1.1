...
class GuiVariator_Arrow extends GuiButton{
    ...
    boolean centeredtextnotset = true;
    public void draw(){
	super.draw();
	if(centeredtextnotset){
	    centeredtextnotset = false;
	    textsettings = new CenteredText(up ? "/\\" : "\\/",getCenter());
	}
	...
	GraphicsDraw.centerText(textsettings);
...
/////////////////////////SecondsVariator
class SecondsVariator extends GuiVariator{
    /////Overrides
    public Color getBaseColor(){
	return _G.COLOR_SECONDSVARIATOR;
    }
	...
	GraphicsDraw.setColor(Color.BLACK);
	GraphicsDraw.biggishFont();
	GraphicsDraw.centerText(""+value+" s",getCenter());
    }
}

/////////////////////////MillisecondsVariator
class MillisecondsVariator extends GuiVariator{
    final int INCREMENTS = 20;
    
    SecondsVariator parent;
    
    public MillisecondsVariator(SecondsVariator par){
	parent = par;
    }
    
    /////Overrides
    public Color getBaseColor(){
	return _G.COLOR_MILLISECONDSVARIATOR;
    }
    ...
    public void onBelowMinValue(){
	if(parent.value != parent.getMinValue()){
	    value = getMaxValue();
	    parent.decrement(1);
	}else{
	    value = getMinValue();
	...
	if(parent.value == parent.getMaxValue() && inc > 0)
	    value = 0;
	else
	    super.increment(inc);
    }
	...
	GraphicsDraw.setColor(Color.BLACK);
	GraphicsDraw.biggishFont();
	GraphicsDraw.centerText(""+(value*1000/INCREMENTS)+" ms",getCenter());
    }
}
